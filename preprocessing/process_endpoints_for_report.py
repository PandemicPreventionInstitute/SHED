...
def generate_lineage_abundance(path, folder_ext, sra_id):
    ...
    try:
        ...
        plt.savefig(path + sra_id + "_lin_abun.png")
        return 0
...
def generate_primer_readcnt(path, folder_ext, sra_id):
    ...
    try:
        primer_read_list.to_csv(path + sra_id + "_primer_readcnt.csv", index=False)
...
def read_cnt_after_filter(path, folder_ext, sra_id):
    ...
    return print(data["summary"]["after_filtering"]["total_reads"])
