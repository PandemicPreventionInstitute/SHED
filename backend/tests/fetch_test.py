...
class TestFetching:
    ...
    # empty filename
        assert fetch.get_fastqs(base_path, '') == 0

    def test_not_a_str_fetch(self):
    # not a string
        assert fetch.get_fastqs(base_path, 2) == 0

    def test_fake_sra_acc_fetch(self):
    # fake SRA accession, error handled by sra toolkit
        assert fetch.get_fastqs(base_path, 'SRR00000001') == 0

    def test_good_sra_acc_fetch(self):
    # good SRA accession
        assert fetch.get_fastqs(base_path, 'SRR17887900') == 0